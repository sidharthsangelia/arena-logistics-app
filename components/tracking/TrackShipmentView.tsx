 ...
 * TRACK SHIPMENT VIEW
 * -----------------------------------------------------------------------------
 * The tracking screen itself. Rendered by both /track (tenant) and
 * /arena-dashboard/track (ops) — one component, because the two pages were
 * byte-identical copies and a fix to one silently left the other behind.
 *
 * The search box takes either number a customer might be holding: Arena's own
 * shipment number (ARN260130748291) or the carrier's AWB. What separates them
 * is decided server-side; this component only renders the answer.
 */
      ...
      {STAGES.map((stage, i) => {
        ...
        return (
          <div key={stage.type} className="flex items-center flex-1 min-w-0">
            ...
            {!isLast && (
              <div className={cn(
                "h-0.5 flex-1 mx-1.5 mb-5.5 rounded-full transition-colors",
...
function ShipmentHeroCard({ result }: { result: CanonicalTrackResult }) {
  ...
  // Legs are only present when the search resolved to one of our bookings.
  // The internal leg is Arena's own record, already obvious from the timeline,
  // so it is not listed as though it were a carrier.
  const carrierLegs = (result.legs ?? []).filter((l) => l.kind !== "internal");
  ...
  return (
    <Card>
      <CardHeader className="pb-5">
        {/* Headline number + badge row */}
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="min-w-0 space-y-1">
            <p className="text-xs font-medium text-muted-foreground">
              {/* Before a waybill exists the headline IS our number, and
                  calling it an airway bill would be wrong. */}
              {shipmentInfo.reference === shipmentInfo.awb
                ? "Shipment Number"
                : "Airway Bill"}
            </p>
        ...
        {/* Our own reference and the route, when the search resolved to a
            booking of ours. A bare carrier AWB has neither. */}
        {(shipmentInfo.reference || shipmentInfo.route) && (
          <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 pt-3">
            {shipmentInfo.reference && shipmentInfo.reference !== shipmentInfo.awb && (
            ...
            {shipmentInfo.route && (
              <div className="flex items-center gap-1.5 min-w-0">
                ...
                <p className="text-xs font-medium truncate">{shipmentInfo.route}</p>
              </div>
            )}
          </div>
        )}
      ...
      <CardContent className="space-y-6 pt-0">
        ...
        {/* Legs. An international booking with door pickup moves on two
            waybills, and a customer holding the pickup label needs to see that
            it belongs to the same journey. */}
        {carrierLegs.length > 0 && (
          <>
            <Separator />
            <div className="space-y-2.5">
              {carrierLegs.map((leg) => (
                <div
                  key={`${leg.kind}-${leg.awb}`}
                ...
                >
                  ...
                  <div className="flex items-baseline gap-2 min-w-0">
                    <p className="text-xs font-mono truncate">{leg.awb}</p>
                    {/* A leg that failed its lookup says so, rather than
                        silently contributing nothing to the timeline. */}
                    {leg.eventCount === 0 && (
                      <span className="text-xs text-muted-foreground whitespace-nowrap">
                        {leg.error ? "no updates" : "awaiting scan"}
                      </span>
                    )}
                  </div>
                </div>
...
interface EventRowProps {
  ...
  showLeg:  boolean; // more than one leg in the timeline — tag each event
}

function EventRow({ event, isLatest, isLast, showLeg }: EventRowProps) {
    ...
    <div className="flex gap-3">
      ...
      <div className={cn("flex-1 min-w-0", isLast ? "pb-0" : "pb-5")}>
        ...
        <div className="flex items-baseline gap-2 flex-wrap">
          ...
          {/* Which leg this came from. Only shown when the timeline actually
              mixes legs, so a single-carrier shipment gains no clutter. */}
          {showLeg && event.legLabel && (
            <span className="text-[10px] font-medium text-muted-foreground border rounded px-1.5 py-0.5 whitespace-nowrap">
              {event.legLabel}
            </span>
          )}
        </div>
...
function TrackingTimeline({
  ...
  // Tag events by leg only when there is more than one to tell apart.
  const showLeg = new Set(events.map((e) => e.legLabel).filter(Boolean)).size > 1;
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center py-16 text-center">
      ...
      <p className="text-sm font-semibold mb-1">Enter a shipment number or AWB</p>
      <p className="text-sm text-muted-foreground max-w-xs">
        Use the Arena shipment number from your booking confirmation
        (ARN260130748291) or the carrier&apos;s airway bill number. Either one
        pulls the same live updates.
      </p>
    </div>
  );
}
...
function LoadingSkeleton() {
  return (
    <div className="space-y-4" aria-label="Loading tracking data">
      {/* Hero card */}
      <Card>
        <CardHeader className="pb-5">
          ...
          <div className="flex items-start gap-2">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="flex items-center flex-1">
                ...
                {i < 4 && <Skeleton className="h-0.5 flex-1 mx-1.5 rounded-none" />}
              </div>
            ))}
          </div>
          <Separator />
...
export function TrackShipmentView() {
    ...
    <div className="max-w-2xl mx-auto px-4 py-8 sm:px-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Enter an Arena shipment number or a carrier AWB to see live updates.
        </p>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder="ARN260130748291 or AWB number"
            className="pl-9 font-mono"
            disabled={isPending}
            aria-label="Shipment number or AWB"
          />
        </div>
        <Button
          onClick={handleTrack}
